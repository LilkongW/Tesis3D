...
# --- NUEVA CONFIGURACIÓN DE SALIDA ---
OUTPUT_GRAPH_DIR = os.path.join(OUTPUT_DIR, "Graficos")
os.makedirs(OUTPUT_GRAPH_DIR, exist_ok=True) # Crear la carpeta Graficos
os.makedirs(OUTPUT_DIR, exist_ok=True) # Asegurar la creación del OUTPUT_DIR
...
def calcular_velocidad_y_aceleracion(df):
    ...
    # CÁLCULO DE POSICIÓN ANGULAR TOTAL
    df['position_angular_total_deg'] = df['angular_dist_deg'].fillna(0).cumsum()
    # ------------------------------------
...
# --- FUNCIÓN DE POSICIÓN ANGULAR TOTAL (MODIFICADA) ---
def generar_visualizacion_posicion_angular_total(df_raw, output_file, stimulus_events=None):
    """
    Genera un gráfico que muestra la Posición Angular Total Acumulada.
    Se modificó el título y el label del eje Y.
    """
    print(f"Generando gráfico de Posición Angular Total en {output_file}...")
    plt.style.use('classic') 
    fig, ax = plt.subplots(figsize=(12, 7)) 

    # Plot: Posición Angular Total Acumulada
    ax.plot(df_raw['timestamp_s'], df_raw['position_angular_total_deg'], 
            label='Posición Angular (°)', color='darkorange', alpha=0.8)
    
    # TÍTULO MODIFICADO
    ax.set_title(f"Posición Angular respecto al Tiempo ({os.path.basename(output_file).replace('_grafico_posicion_angular.png', '')})")
    # LABEL DEL EJE Y MODIFICADO
    ...
    if stimulus_events:
        for i, event in enumerate(stimulus_events):
            ...
            ax.axvline(x=start_s, color=color, linestyle='--', alpha=0.7, zorder=1)

    ax.legend(loc='upper left')
    plt.tight_layout()
    plt.savefig(output_file)
    print(f"Gráfico de Posición Angular Total guardado en: {output_file}")
    plt.close(fig)
# -----------------------------------------------------


def generar_visualizacion_solo_velocidad(df_raw, umbrales, output_file, stimulus_events=None):
    """
    Genera un gráfico que muestra ÚNICAMENTE la Velocidad Angular Filtrada y el Umbral de Fijación.
    """
    print(f"Generando gráfico de SOLO Velocidad en {output_file}...")
    plt.style.use('classic') 
    # Usamos (12, 7) para mantener la consistencia cuadrada
    fig, ax = plt.subplots(figsize=(12, 7)) 

    # Plot: Velocidad Angular Filtrada
    ax.plot(df_raw['timestamp_s'], df_raw['velocity_angular_filtered'], label='Velocidad Angular Filtrada (°/s)', color='blue', alpha=0.7)
    
    # Umbral de Fijación
    ax.axhline(y=umbrales['V_MAXIMA_FIJACION'], color='green', linestyle=':', label=f"Umbral Fijación ({umbrales['V_MAXIMA_FIJACION']} °/s)")
    
    ax.set_title(f"Velocidad Angular 3D Filtrada ({os.path.basename(output_file).replace('_grafico_solo_velocidad.png', '')})")
    ...
    plt.tight_layout()
    plt.savefig(output_file)
    print(f"Gráfico de SOLO Velocidad guardado en: {output_file}")
    plt.close(fig)


def generar_visualizacion_posicion(df_raw, events_report, umbrales, output_file, stimulus_events=None):
    print(f"Generando gráfico de Posición en {output_file}...")
    plt.style.use('classic') # <--- Estilo Clásico
    # MODIFICADO: Ajuste de figsize a (12, 7)
    fig, ax = plt.subplots(figsize=(12, 7)) 
    event_color_map = {'fijacion_valida': 'green', 'sacadico': 'red', 'parpadeo': 'gray'}

    # Subplot 1: Posición
    # CORRECCIÓN APLICADA AQUÍ: Componente y y z ahora usan linestyle='-' (línea continua)
    ax.plot(df_raw['timestamp_s'], df_raw['gaze_x'], label='Componente x', alpha=0.8, linestyle='-')
    ax.plot(df_raw['timestamp_s'], df_raw['gaze_y'], label='Componente y', alpha=0.8, linestyle='-') 
    ax.plot(df_raw['timestamp_s'], df_raw['gaze_z'], label='Componente z', alpha=0.6, linestyle='-')
    ax.set_title(f"Posición del Vector de Mirada 3D ({os.path.basename(output_file).replace('_grafico_posicion.png', '')})")
    ax.set_ylabel('Componente del Vector')
    ax.grid(True)

    # Dibujar regiones de eventos (parpadeo)
    for _, event in events_report.iterrows():
        color = event_color_map.get(event['event_class'])
        if color:
            if event['event_class'] == 'parpadeo':
                ax.axvspan(event['start_time_ms'] / 1000.0, event['end_time_ms'] / 1000.0, color=color, alpha=0.4)

    # Dibujar marcas de estímulos (Líneas Verticales)
    if stimulus_events:
        y_lim_ax = ax.get_ylim()
        for i, event in enumerate(stimulus_events):
            color = 'red' if i % 2 == 0 else 'magenta'
            start_s = event['start_time_ms'] / 1000.0
            ax.axvline(x=start_s, color=color, linestyle='--', alpha=0.7, zorder=1)
            ax.text(start_s + 0.05, y_lim_ax[1] * 0.9, event['label'], 
                     rotation=90, verticalalignment='top', color=color, fontsize=9, weight='bold')
        ax.set_ylim(y_lim_ax)

    ax.legend(loc='lower right') # <--- Posición de la Leyenda
    ax.set_xlabel('Tiempo (s)')
    plt.tight_layout()
    plt.savefig(output_file)
    print(f"Gráfico de Posición guardado en: {output_file}")
    plt.close(fig)

def generar_visualizacion_velocidad(df_raw, events_report, umbrales, output_file, stimulus_events=None):
    print(f"Generando gráfico de Velocidad en {output_file}...")
    plt.style.use('classic') # <--- Estilo Clásico
    # MODIFICADO: Ajuste de figsize a (12, 7)
    fig, ax = plt.subplots(figsize=(12, 7)) 
    event_color_map = {'fijacion_valida': 'green', 'sacadico': 'red', 'parpadeo': 'gray'}
    ...
    ax.plot(df_raw['timestamp_s'], df_raw['velocity_angular_filtered'], label='Velocidad Angular Filtrada (°/s)', color='blue', alpha=0.7)
    ax.axhline(y=umbrales['V_MAXIMA_FIJACION'], color='green', linestyle=':', label=f"Umbral Fijación ({umbrales['V_MAXIMA_FIJACION']} °/s)")
    ax.set_title(f"Velocidad Angular 3D y Eventos Oculares Detectados ({os.path.basename(output_file).replace('_grafico_velocidad.png', '')})")
    ax.set_ylabel('Velocidad Angular (°/s)')
    ax.grid(True)
    ax.set_xlabel('Tiempo (s)')
    ...
    for _, event in events_report.iterrows():
        color = event_color_map.get(event['event_class'])
        if color:
            ax.axvspan(event['start_time_ms'] / 1000.0, event['end_time_ms'] / 1000.0, color=color, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(output_file)
    print(f"Gráfico de Velocidad guardado en: {output_file}")
    plt.close(fig)

def generar_visualizacion_aceleracion(df_raw, events_report, umbrales, output_file, stimulus_events=None):
    print(f"Generando gráfico de Aceleración en {output_file}...")
    plt.style.use('classic') # <--- Estilo Clásico
    # MODIFICADO: Ajuste de figsize a (12, 7)
    fig, ax = plt.subplots(figsize=(12, 7)) 
    event_color_map = {'fijacion_valida': 'green', 'sacadico': 'red', 'parpadeo': 'gray'}
    ...
    ax.plot(df_raw['timestamp_s'], df_raw['acceleration_angular_filtered'], label='Aceleración Angular Filtrada (°/s²)', color='purple', alpha=0.7)
    ax.axhline(y=umbrales['A_PICO_MINIMO'], color='orange', linestyle=':', label=f"Umbral Acel. Pos. ({umbrales['A_PICO_MINIMO']} °/s²)")
    ax.axhline(y=-umbrales['A_PICO_MINIMO'], color='orange', linestyle=':', label=f"Umbral Acel. Neg. ({-umbrales['A_PICO_MINIMO']} °/s²)")
    ax.set_title(f"Aceleración Angular 3D y Detección de Sacádicos (I-AT) ({os.path.basename(output_file).replace('_grafico_aceleracion.png', '')})")
    ax.set_xlabel('Tiempo (s)')
    ...
    plt.tight_layout()
    plt.savefig(output_file)
    print(f"Gráfico de Aceleración guardado en: {output_file}")
    plt.close(fig)

# --- NUEVA FUNCIÓN AÑADIDA ---
def generar_visualizacion_solo_aceleracion(df_raw, umbrales, output_file, stimulus_events=None):
    """
    Genera un gráfico que muestra ÚNICAMENTE la Aceleración Angular Filtrada y los Umbrales Sacádicos.
    """
    print(f"Generando gráfico de SOLO Aceleración en {output_file}...")
    plt.style.use('classic') 
    fig, ax = plt.subplots(figsize=(12, 7)) 

    # Plot: Aceleración Angular Filtrada
    ax.plot(df_raw['timestamp_s'], df_raw['acceleration_angular_filtered'], label='Aceleración Angular Filtrada (°/s²)', color='purple', alpha=0.7)
    
    # Umbrales de Sacádicos (I-AT)
    umbral_pos = umbrales['A_PICO_MINIMO']
    umbral_neg = -umbrales['A_PICO_MINIMO']
    
    ax.axhline(y=umbral_pos, color='orange', linestyle=':', 
               label=f"Umbral Acel. Pos. ({umbral_pos} °/s²)")
    ax.axhline(y=umbral_neg, color='orange', linestyle=':', 
               label=f"Umbral Acel. Neg. ({umbral_neg} °/s²)")
    
    ax.set_title(f"Aceleración Angular respecto al Tiempo ({os.path.basename(output_file).replace('_grafico_solo_aceleracion.png', '')})")
    ax.set_ylabel('Aceleración Angular (°/s²)')
    ax.set_xlabel('Tiempo (s)')
    ax.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(output_file)
    print(f"Gráfico de SOLO Aceleración guardado en: {output_file}")
    plt.close(fig)
# ------------------------------------

# --- NUEVAS FUNCIONES PARA GRÁFICOS DE VELOCIDAD INDIVIDUALES ---

def generar_visualizacion_velocidad_fijaciones(df_raw, events_report, umbrales, output_file, stimulus_events=None):
    """
    Genera un gráfico que muestra ÚNICAMENTE la Velocidad Angular con Fijaciones detectadas.
    Leyenda en la esquina superior derecha.
    """
    print(f"Generando gráfico de Velocidad con Fijaciones en {output_file}...")
    plt.style.use('classic')
    fig, ax = plt.subplots(figsize=(12, 7))
    event_color_map = {'fijacion_valida': 'green', 'parpadeo': 'gray'}

    # Plot: Velocidad Angular Filtrada
    ax.plot(df_raw['timestamp_s'], df_raw['velocity_angular_filtered'], 
            label='Velocidad Angular Filtrada (°/s)', color='blue', alpha=0.7, zorder=1)
    
    # Umbral de Fijación
    ax.axhline(y=umbrales['V_MAXIMA_FIJACION'], color='green', linestyle=':', 
               label=f"Umbral Fijación ({umbrales['V_MAXIMA_FIJACION']} °/s)")
    
    ax.set_title(f"Velocidad Angular y Fijaciones Detectadas ({os.path.basename(output_file).replace('_grafico_velocidad_fijaciones.png', '')})")
    ax.set_xlabel('Tiempo (s)')
    ax.set_ylabel('Velocidad Angular (°/s)')
    ax.grid(True)
    ...
    for _, event in fix_blink_report.iterrows():
        color = event_color_map.get(event['event_class'])
        if color:
            ax.axvspan(event['start_time_ms'] / 1000.0, event['end_time_ms'] / 1000.0, 
    ...
    ax.legend(loc='upper right')  # Leyenda arriba a la derecha
    plt.tight_layout()
    plt.savefig(output_file)
    print(f"Gráfico de Velocidad con Fijaciones guardado en: {output_file}")
    plt.close(fig)


def generar_visualizacion_velocidad_sacadicos(df_raw, events_report, umbrales, output_file, stimulus_events=None):
    """
    Genera un gráfico que muestra ÚNICAMENTE la Velocidad Angular con Sacádicos detectados.
    Leyenda en la esquina superior derecha.
    """
    print(f"Generando gráfico de Velocidad con Sacádicos en {output_file}...")
    plt.style.use('classic')
    fig, ax = plt.subplots(figsize=(12, 7))
    ...
    ax.set_title(f"Velocidad Angular y Sacádicos Detectados ({os.path.basename(output_file).replace('_grafico_velocidad_sacadicos.png', '')})")
    ax.set_xlabel('Tiempo (s)')
    ax.set_ylabel('Velocidad Angular (°/s)')
    ax.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(output_file)
    print(f"Gráfico de Velocidad con Sacádicos guardado en: {output_file}")
    plt.close(fig)
...
def generar_visualizacion_fijaciones(df_raw, events_report, umbrales, output_file, stimulus_events=None):
    """
    MODIFICADO: Usa líneas verticales para estímulos, no regiones. Estilo 'classic' y leyenda 'lower right'.
    """
    print(f"Generando gráfico de fijaciones en {output_file}...")
    plt.style.use('classic') # <--- Estilo Clásico
    # MODIFICADO: Ajuste de figsize a (12, 10)
    ...
    # Subplot 1: Posición
    # CORRECCIÓN APLICADA AQUÍ: Componente y y z ahora usan linestyle='-' (línea continua)
    ax1.plot(df_raw['timestamp_s'], df_raw['gaze_x'], label='Componente x', alpha=0.8, zorder=1, linestyle='-')
    ax1.plot(df_raw['timestamp_s'], df_raw['gaze_y'], label='Componente y', alpha=0.8, zorder=1, linestyle='-')
    ax1.plot(df_raw['timestamp_s'], df_raw['gaze_z'], label='Componente z', alpha=0.6, zorder=1, linestyle='-')
    ax1.set_title(f"Posición de Mirada y Fijaciones Detectadas ({os.path.basename(output_file).replace('_grafico_fijaciones.png', '')})")
    ax1.set_ylabel('Componente del Vector')
    ax1.legend(loc='lower right') # <--- Posición de la Leyenda
    ax1.grid(True)
    ...
    ax2.legend(loc='lower right') # <--- Posición de la Leyenda
    ax2.grid(True)
...
def generar_visualizacion_sacadicos(df_raw, events_report, umbrales, output_file, stimulus_events=None):
    ...
    ax1.plot(df_raw['timestamp_s'], df_raw['gaze_y'], label='Componente y', alpha=0.8, zorder=1, linestyle='-')
    ax1.plot(df_raw['timestamp_s'], df_raw['gaze_z'], label='Componente z', alpha=0.6, zorder=1, linestyle='-')
    ax1.set_title(f"Posición de Mirada y Sacádicos ({os.path.basename(output_file).replace('_grafico_sacadicos.png', '')})")
    ax1.set_ylabel('Componente del Vector')
    ax1.legend(loc='lower right') # <--- Posición de la Leyenda
    ax1.grid(True)
...
def generar_histograma_velocidad(df_raw, umbrales, output_file):
    ...
    ax.set_title(f"Histograma de Velocidad Angular (Escala Log) ({os.path.basename(output_file).replace('_histograma_velocidad.png', '')})")
...
def generar_histograma_aceleracion(df_raw, umbrales, output_file):
    ...
    ax.set_title(f"Histograma de Aceleración Angular (Escala Log) ({os.path.basename(output_file).replace('_histograma_aceleracion.png', '')})")
...
def main():
    ...
    for input_file_path in lista_archivos_csv:
        ...
        # --- NUEVOS ARCHIVOS DE SALIDA EN LA CARPETA GRAFICOS ---
        output_plot_pos = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_posicion.png")
        output_plot_vel = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_velocidad.png")
        output_plot_acel = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_aceleracion.png")
        output_plot_solo_vel = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_solo_velocidad.png")
        output_plot_pos_angular = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_posicion_angular.png")
        output_plot_fix = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_fijaciones.png")
        output_plot_sac = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_sacadicos.png")
        output_plot_hist_vel = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_histograma_velocidad.png")
        output_plot_hist_acel = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_histograma_aceleracion.png")
        output_plot_solo_acel = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_solo_aceleracion.png")
        
        # NUEVOS GRÁFICOS DE VELOCIDAD INDIVIDUAL
        output_plot_vel_fix = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_velocidad_fijaciones.png")
        output_plot_vel_sac = os.path.join(OUTPUT_GRAPH_DIR, f"{base_filename}_grafico_velocidad_sacadicos.png")
        # --- FIN NUEVOS ARCHIVOS DE SALIDA ---
        ...
        try:
            ...
            parpadeos_df.to_csv(output_report_blink, index=False)
        ...
        # 7. Generar Gráficos
        try:
            # Gráfico de Posición Angular Total (MODIFICADO)
            generar_visualizacion_posicion_angular_total(df_completo, output_plot_pos_angular, stimulus_events=eventos_estimulo)
            
            # Gráficos anteriores
            generar_visualizacion_posicion(df_completo, reporte_final_combinado, UMBRALES_3D, output_plot_pos, stimulus_events=eventos_estimulo)
            generar_visualizacion_velocidad(df_completo, reporte_final_combinado, UMBRALES_3D, output_plot_vel, stimulus_events=eventos_estimulo)
            generar_visualizacion_aceleracion(df_completo, reporte_final_combinado, UMBRALES_3D, output_plot_acel, stimulus_events=eventos_estimulo)
            generar_visualizacion_solo_velocidad(df_completo, UMBRALES_3D, output_plot_solo_vel, stimulus_events=eventos_estimulo)
            ...
            generar_visualizacion_solo_aceleracion(df_completo, UMBRALES_3D, output_plot_solo_acel, stimulus_events=eventos_estimulo)
            
            # NUEVOS GRÁFICOS DE VELOCIDAD INDIVIDUAL
            generar_visualizacion_velocidad_fijaciones(df_completo, reporte_final_combinado, UMBRALES_3D, output_plot_vel_fix, stimulus_events=eventos_estimulo)
            generar_visualizacion_velocidad_sacadicos(df_completo, reporte_final_combinado, UMBRALES_3D, output_plot_vel_sac, stimulus_events=eventos_estimulo)
    ...
    try:
        ...
        print(f"Análisis 3D (I-AT) completado. Se generaron reportes en: {OUTPUT_DIR}")
        print(f"Los gráficos están en la subcarpeta: {OUTPUT_GRAPH_DIR}")
